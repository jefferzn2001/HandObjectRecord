...
def _cluster_poses_py(angle_diff_deg, dist_diff, poses, symmetry_tfs):
  """Pure-Python fallback for mycpp.cluster_poses (avoids C++ segfault)."""
  ...
  for i in range(1, len(poses)):
    ...
    is_new = True
    for cluster in out:
      t0, t1 = cluster[:3, 3], cur[:3, 3]
      if np.linalg.norm(t0 - t1) >= dist_diff:
        continue
      for tf in symmetry_tfs:
        cur_tmp = cur @ tf
        R_diff = cur_tmp[:3, :3] @ cluster[:3, :3].T
        cos_val = np.clip((np.trace(R_diff) - 1) / 2.0, -1.0, 1.0)
        if np.arccos(cos_val) < radian_thres:
          is_new = False
          break
      if not is_new:
        break
...
class FoundationPose:
  ...
  def make_rotation_grid(self, min_n_views=40, inplane_step=60):
    ...
    sym_tfs = self.symmetry_tfs.data.cpu().numpy()
    if mycpp is not None:
      try:
        rot_grid = np.asarray(mycpp.cluster_poses(30, 99999, rot_grid, sym_tfs))
      except Exception:
        rot_grid = _cluster_poses_py(30, 99999, rot_grid, sym_tfs)
    else:
      rot_grid = _cluster_poses_py(30, 99999, rot_grid, sym_tfs)
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    poses = poses[::2]        # keep every 2nd pose  -> ~half memory
    logging.info(f"subsampled poses for scorer: {poses.shape}")
